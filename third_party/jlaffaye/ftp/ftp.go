...
// isTransientDataConnErr reports whether a data-connection dial error is worth
// one retry (connection refused / reset — server passive listener races).
func isTransientDataConnErr(err error) bool {
	if err == nil {
		return false
	}
	var netErr net.Error
	if errors.As(err, &netErr) && netErr.Timeout() {
		return false // 超时通常意味着防火墙丢包，重试无意义
	}
	return strings.Contains(strings.ToLower(err.Error()), "refused") ||
		strings.Contains(strings.ToLower(err.Error()), "reset by peer")
}
...
func (c *ServerConn) cmdDataConnFrom(offset uint64, format string, args ...interface{}) (net.Conn, error) {
	...
	// Spark 补丁：部分服务器被动端口偶发"拒绝连接"（监听器开启稍慢 / 端口抖动），
	// 重新发起 EPSV/PASV 重试一次
	var conn net.Conn
	var err error
	for attempt := 0; ; attempt++ {
		...
		if err != nil {
			if attempt == 0 && isTransientDataConnErr(err) {
	...
	// Spark 补丁：部分 FTP 服务器（如某些 NAS / 嵌入式设备）对 RETR / LIST 等
	// 数据命令回复 200 "Ready to proceed" 而不是标准的 125/150，这里一并接受。
	if code != StatusAlreadyOpen && code != StatusAboutToSend && code != StatusCommandOK {
		_ = conn.Close()
...
func (c *ServerConn) checkDataShut() error {
	...
	// Spark 补丁：接受 200 作为传输完成应答（部分服务器始终用 200）
	code, _, err := c.conn.ReadResponse(-1)
	if err != nil {
		return err
	}
	if code != StatusClosingDataConnection && code != StatusCommandOK {
		return &textproto.Error{Code: code, Msg: "unexpected transfer completion status"}
	}
	return nil
}
